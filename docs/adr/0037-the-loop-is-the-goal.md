...
| a C compiler linked in process (libtcc is the instance) | a C library at run time | a compile, no process | still unmeasured *linked*; the same compiler as a process is `benches/c-floor/observation-tcc.txt` | unoptimised, and now read: 40x the optimising compiler on the integer kernel, under 6x on the state kernel, and it will not run this tier's corpus unless the inlining is turned off with it (`observation-loop-profile.txt`) | one code generator; a second C toolchain to bind |
