...
| remembering a pure nullary root, which the in-process tier already did | **about six percent off the minimum**, and taken. Not a lever so much as a parity gap: the kernel's input is a 65KB literal and `rt_lit` rebuilds a literal every evaluation. Six percent is what it is worth *here*, where the kernel reads its input once; the shape it is worth two orders of magnitude on is a root called inside a loop, which this kernel is not and the front end has few of |
