...
**Amended by ADR 0032 §3–5.** The `Bfo` arm above was taken by narrowing
`Reference`, **because no real code generator could be put in that position.**
One can now, and it reproduces this arm's entry line on cranelift rather than on
a narrowed tree-walker — **and it reads 0.604×, not 2.104×.**

**The arithmetic here is not withdrawn; what it prices is. 2.10× is an
*infinitely fast* backend's number at that entry count, and no real backend
reaches it, because the boundary is paid every time whatever is on the far
...
thousands of entries into dozens and pays.
