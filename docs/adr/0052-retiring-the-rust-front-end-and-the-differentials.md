...
and not yet exercised. The merges after it read 126 s, 130 s, 142 s, 168 s and 163 s, each reusing by tree the same way and for the same reason. The last is twelve seconds under the bound, which reads as a drift and is not one: over the last nine green runs the longest partition has been 88, 91, 97, 101, 101, 102, 110, 113 and 120 s, a band with no step where §2's 38 MB of text goldens landed. A draft of this sentence called it a trend, from four partitions in one run's longest-five rather than from the series. Six running have hit the lookup directly, because none of them moved main while
