...
mutations=(
"items.ply|s/constraints: cs.node,/constraints: [],/|a DROPPED FIELD: a fn's \`where\` constraints are parsed and thrown away|every list emits its length"
...
"spine.ply|s/Stop({p: e.p, node: push(s.out, r.node)})/Stop({p: e.p, node: s.out})/|a DROPPED LIST ELEMENT: every comma list loses its last member|every list emits its length"
"types.ply|s/{ name: n.node, ty: Some(t.node),/{ name: n.node, ty: None,/|a DROPPED OPTION: a parameter's type annotation is parsed and discarded|every Option emits its presence"
"spine.ply|s/if p.last_code == d.code \&\& p.last_span.start == s.start \&\& p.last_span.end == s.end {/if p.last_code == d.code {/|a WIDENED DEDUP: two diagnostics with one code at two places become one|the diagnostic list, and its length"
...
"spine.ply|s/node: qualified(first.node, second.node) }/node: bare(second.node) }/|a COLLAPSED QUALIFIER: \`store::place\` loses its module and becomes \`place\`|the Option inside every QName, and that name node's span"
...
"types.ply|s/node: { eff: ef, mode: m.node, resource: r.node,/node: { eff: ef, mode: m.node, resource: None,/|a DROPPED FIELD inside an effect row: an atom loses its resource label|the row atoms desk.ply's projection compares -- clause 2 of the expander tolerance"
