...
use ply_parser_spike_harness::{
    byte_literal, programs, records, reference_check_dump, reference_check_dump_known,
};
...
impl Project {
    ...
    /// One `ply run` for many programs, each a list of `(name, source)` modules, through the
    /// named `infer` entry point.
    fn dumps_through(&self, entry: &str, inputs: &[Vec<(String, String)>]) -> Vec<String> {
        let mut src = format!(
...
fn compare(label: &str, inputs: &[(String, Vec<(String, String)>)]) {
    compare_through("check_dump", reference_check_dump, label, inputs);
}
...
fn compare_through(
    ...
    let got = project.dumps_through(entry, &programs);
