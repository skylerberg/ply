...
impl Project {
    ...
    fn dumps_in(&self, names: &[String], inputs: &[Vec<u8>], ctors: &[String]) -> Vec<String> {
        ...
        for (i, bytes) in inputs.iter().enumerate() {
            ...
            parts.push(format!(
                "bytes_of_string(emit_dump_in(n{i}(), ctors(), builtins(), s{i}()))"
            ));
    ...
    /// The producer's rendering: every body of every input, framed by length, as one string.
    fn bodies_in(&self, names: &[String], inputs: &[Vec<u8>], ctors: &[String]) -> String {
        let mut src = String::from(
            "// Generated by spikes/ply-parser/harness/tests/emit_diff.rs. Not checked in.\n\
             import emit (emit_bodies_in)\n",
        );
        src.push_str(&format!(
            "fn builtins() -> List<Bytes> = [{}]\nfn ctors() -> List<Bytes> = [{}]\n",
            ply_parser_spike_harness::reference_builtins()
        ...
        let mut parts: Vec<String> = Vec::new();
        for (i, (name, bytes)) in names.iter().zip(inputs).enumerate() {
            src.push_str(&format!("fn s{i}() -> Bytes = {}\n", byte_literal(bytes)));
            src.push_str(&format!(
                "fn n{i}() -> Bytes = {}\n",
            ...
            parts.push(format!(
                "bytes_of_string(emit_bodies_in(n{i}(), ctors(), builtins(), s{i}()))"
            ));
        }
        src.push_str(&format!(
            "fn main() -> String = string_of_bytes(bytes_concat_all([{}]))\n",
...
fn the_port_agrees_with_the_reference_over_the_shipped_corpus() {
    ...
    //
    // The other three are single bodies with causes of their own, none yet run down:
    // `std.db.collect_tuple` and `std.http.absorb` release an update's base where the port does
    // not; `std.http.check_limits` emits a constant the port does not reach.
    let expected_gaps = [
