...
fn a_unique_close_costs_the_allocator_nothing_even_beside_a_live_continuation() {
    ...
    // A continuation live over a region opened after its capture, as a handler's scratch cell is.
...
fn a_later_close_reclaims_what_an_earlier_one_could_not() {
    ...
    // The next close reaps the dead continuation first, so it accounts only for its own run.
...
/// The marks cannot tell this region from one open at the capture; the open ordinals can.
#[test]
...
fn a_continuation_parked_in_the_region_that_pins_it_is_the_leak_adr_0017_accepts() {
    ...
    // Reachable only from a slot of the region it holds open, so nothing outside drops it.
