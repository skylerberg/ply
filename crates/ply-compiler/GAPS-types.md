...
generics, effect rows, parameters and patterns. `crates/ply-compiler/GAPS-lexer.md` is
...
`crates/ply-compiler/GAPS-lexer.md` §10 files the `List` surface —
...
`crates/ply-compiler/GAPS-lexer.md` §1 is the lexer spike's headline: a growing
...
`crates/ply-compiler/GAPS-lexer.md` §15 records the lexer at **668 / 1,069 = 0.62**.
...
- **Structural equality on ADT values.** `at(c, p, t_colon())` is
  ...
  dispatch. `crates/ply-compiler/GAPS-lexer.md` §13's "no dispatch mechanism, and it did
