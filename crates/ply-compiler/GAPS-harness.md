...
Written in `crates/ply-compiler/GAPS-lexer.md`'s style: every count in it was taken on
...
4. **`Lit::Decimal`'s `mantissa` and `scale`, and `Lit::Float`'s value.** Both
   ...
   `crates/ply-compiler-diff` makes for floats, made here in the direction that
...
`crates/ply-compiler/GAPS-lexer.md` §1 and §10 describe is real and it is **not
...
| --- | ---: | ---: | ---: |
| `crates/ply-compiler-diff/lexer.ply` | 619 | 370 | 58 |
