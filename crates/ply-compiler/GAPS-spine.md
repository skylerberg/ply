...
`crates/ply-compiler/GAPS-lexer.md` already recorded, it says so and reports **what
...
| `lexer.rs` against `crates/ply-compiler-diff/lexer.ply`, code lines | 950 | 420 | 0.44 |
...
`crates/ply-compiler/GAPS-lexer.md` §12's finding was that error accumulation cost the
...
`crates/ply-compiler/GAPS-lexer.md` §2: no record update syntax, so changing one field of
`P` means writing all nine. `crates/ply-compiler/GAPS-lexer.md` §1, **measured**: a
...
`crates/ply-compiler/GAPS-lexer.md` §11 saw the same shape on the lexer's `at` and called
...
2. **The budget is a backstop that cannot fire**, in
   ...
   of the loop, `crates/ply-compiler/GAPS-lexer.md` §5 — and builds the `range` list of
...
`X0001` (`crates/ply-compiler/README.md` §"Where this disagrees on purpose"), so an
...
the lexing, which `crates/ply-compiler-diff` already established. desk.ply's 19,576
matches `crates/ply-compiler/GAPS-lexer.md` §5 and §15 exactly.
...
**So `crates/ply-compiler/GAPS-lexer.md` §1's positional tax does not visibly apply to a
