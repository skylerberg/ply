# `crates/ply-compiler` — Ply's compiler, written in Ply

**This is what every run compiles through.** Under tier-only (ADR 0048) the C
...
**It was a spike, and the rest of this file was written while it was one.** The
history below is kept because it is the record of what was measured and why; read
the paragraph above for what is true now.
...
> from `crates/ply-compiler-diff`; byte-identical, and needed no edit."* It needed one:
...
> that arm, a two-line comment and one test. **`crates/ply-compiler-diff` still has the
> hole**, and worse: as of 2026-08-30 `crates/ply-compiler-diff/run.sh` reaches no test
...
./crates/ply-compiler-diff/tools/arm.sh --arm              # and then the sixteen mutations
./crates/ply-compiler-diff/tools/measure-front-end.sh      # GAPS.md §13's series, re-taken
./crates/ply-compiler-diff/tools/measure-multiplier.sh     # GAPS-harness.md §H5's series
...
The same reason `crates/ply-compiler-diff/Cargo.toml` gives, and with the same
...
> `./crates/ply-compiler-diff/tools/arm.sh` on every pull request, gated on `fmt` and
...
> `probes/` the way it covers `crates/`: a directory there that no job runs and
> no entry excuses fails the `plan` job. That is what would have caught this in
> the first place, and it is what now says out loud that `crates/ply-compiler-diff` is
...
- `./crates/ply-compiler-diff/tools/arm.sh --arm` exits **0**.
...
- **No source file outside `crates/ply-compiler/` was modified.** Taking §4's
  measurements created three gitignored artifacts and nothing else:
  `examples/.ply-cache` and `crates/ply-compiler/.ply-cache` (written by `ply
  check` / `ply run`, removed before every timed run) and
  `crates/ply-compiler-diff/target/`, from building that spike's existing
