...
In the style of `crates/ply-compiler/GAPS-lexer.md`. Each entry says what I was trying to
...
- **building the map** = M − L = **0.03 s** — 14,742 `map_insert`s, 11% of the
  ...
  shares structure rather than copying; `crates/ply-compiler/GAPS-lexer.md` §1's
...
2. **Ten functions in this module violated the invariant on the first write**,
   ...
   specific new hazard the design creates**, and `crates/ply-compiler/GAPS-lexer.md` §12
...
`{..p, pos: n}` does not exist (`crates/ply-compiler/GAPS-lexer.md` §2), so every state
...
`crates/ply-compiler/GAPS-lexer.md` §9 counted three record types in a lexer that existed
...
`crates/ply-compiler/GAPS-lexer.md` §15 reports "roughly an order of magnitude slower"
...
2. **Record update syntax.** §6, and it compounds with
   `crates/ply-compiler/GAPS-lexer.md` §1 exactly as that entry warned: the workaround for
