...
Written in the style of `crates/ply-compiler/GAPS-lexer.md`: what I was trying to
...
`crates/ply-compiler/GAPS-lexer.md` §10 files "no index, no `nth`, no `last`" as
...
`crates/ply-compiler/GAPS-lexer.md` §12) and it is only available because the reference's
...
`crates/ply-compiler/GAPS-lexer.md` §9 counted three types in the lexer that existed only
...
`crates/ply-compiler/GAPS-lexer.md` §1's rule is that a growing container must be the
...
`crates/ply-compiler/GAPS-lexer.md` §1 column 3 shows doubles the recursion depth.
...
`crates/ply-compiler/GAPS-lexer.md` and ADR 0020 §6.2 both lean on the lexer's line
