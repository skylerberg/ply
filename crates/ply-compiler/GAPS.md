...
`crates/ply-compiler/GAPS-lexer.md` §10 files the `List` surface — `len, push, map,
...
across seven functions, `crates/ply-compiler/GAPS-lexer.md` §12).
...
`crates/ply-compiler/GAPS-lexer.md` §9 counted **three** record types in the lexer that
...
## §4 No record update, and `crates/ply-compiler/GAPS-lexer.md` §1's field-order rule stops being local
...
(`crates/ply-compiler/GAPS-lexer.md` §2), so changing one field of `P` means writing all
...
**The field-order rule.** `crates/ply-compiler/GAPS-lexer.md` §1, measured: a growing
...
3. **A loop that grows two lists at once has no correct spelling.** A record
   ...
   escape — splitting the loop in two — is the one `crates/ply-compiler/GAPS-lexer.md` §1
...
`crates/ply-compiler/GAPS-lexer.md` §11 saw this on the lexer's `at` and called it "a
...
(`crates/ply-compiler/GAPS-lexer.md` §7), so `spine.ply`'s is ASCII `A`..`Z`.
...
`crates/ply-compiler/README.md`, and this is not a second one.
...
3. **`Lit::Decimal`'s mantissa and scale, and `Lit::Float`'s value.** Both sides
   ...
   nor an `i128` from digits (`crates/ply-compiler/GAPS-lexer.md` §3, §4), so a dump
...
**`./crates/ply-compiler-diff/tools/arm.sh` exits 101 at the differential.** 28 of the 763
...
| what `crates/ply-compiler/*.ply` must learn | where | est. Ply lines |
| --- | --- | ---: |
...
> Withdrawn: *"**`./crates/ply-compiler-diff/tools/arm.sh` exits 101 at the differential.**"*
...
| --- | ---: | ---: | ---: |
| `crates/ply-compiler-diff/lexer.ply` | 618 | 370 | 58 |
...
reproduces §14 exactly on `crates/ply-compiler-diff/lexer.ply` (618/370/58), `spine.ply`
...
| delta | **+39** | **−145** | **+16** |
| `crates/ply-compiler-diff/lexer.ply` (untouched) | 618 | 370 | 58 |
...
- **Structural equality on ADT values**, cross-module, with no `derive` and no
  ...
  `crates/ply-compiler/GAPS-lexer.md` §13's "no dispatch mechanism, and it did not bite"
...
*this `push` will copy*. `crates/ply-compiler/GAPS-lexer.md` said so, ADR 0020 §7 agreed,
...
`mine-checks.py` mines from `crates/ply-core-tests/tests/suite/unit/infer.rs` the way
...
**One value is out of reach**, and it is `crates/ply-compiler/GAPS-lexer.md`'s: a `U64`
