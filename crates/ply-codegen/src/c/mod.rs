...
//! | `PLY_C_BOOTSTRAP` | `off` builds the Ply emitter with the reference emitter instead of from `crates/ply-compiler/bootstrap`, the bundle of the C it last emitted for itself; the fixpoint test in `crates/ply-codegen-tests` is what says the bundle serves, and `PLY_C_BOOTSTRAP_REFRESH=1` on that test rewrites it | `bundle.rs` |
