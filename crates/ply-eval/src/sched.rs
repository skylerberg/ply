...
//! bottom of this file checks the part of the rule that is greppable.
//!
...
/// Why a task cannot run. Both are answers to *enabledness*, which is a
...
impl Scheduler {
    /// `span` is the `simulate` region's own span, which is what a deadlock is
    /// reported against.
    ...
    pub fn spawn(&mut self, body: Value, over: Vec<Delimiter>, span: Span) -> TaskId {
        ...
        // the child does, so the child starts from the parent's clock.
        let inherited = match self.current {
            Some(parent) => self.clocks[parent.0 as usize].clone(),
            None => Vec::new(),
        };
    ...
    fn internal(&self, message: impl Into<String>) -> Diagnostic {
        Diagnostic::error(codes::INTERNAL_ERROR, message)
            .primary(self.span, "the simulated scheduler was driven out of order")
    }
}
...
mod tests {
    ...
    fn suspended() -> Continuation {
        ...
        Stack::new().push_prompt(prompt).capture(1).0
    }
