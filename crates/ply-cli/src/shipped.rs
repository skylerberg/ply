...
//! `compiler.<name>`, and the `ply fmt` program built from `crates/ply-cli/ply`.
//!
...
/// The source of the program `ply fmt` runs.
pub const FMT_SOURCE: &str = include_str!("../ply/fmt.ply");

/// The module `ply/fmt.ply` is, which decides the entry point's name.
const FMT_MODULE: &str = "fmt";
...
pub const ARTIFACT: &str = "fmt.plyx";

pub const DIGEST: &str = "fmt.digest";
...
/// Where a program built for `identity` is kept between runs, beside the emitter's own stages.
fn stage() -> PathBuf {
...
/// The `ply fmt` program: the committed artifact when it was built from these very sources, else
/// the stage an earlier run kept for them, else one built now and kept there. A binary whose
/// committed artifact is behind its sources therefore runs the sources, never the artifact.
...
/// The program built from its source here and now, as `ply build` would build it: written into a
/// directory of its own, loaded whole, and closed over its `main`.
pub fn build() -> Result<Vec<u8>, Diagnostic> {
    let dir = stage().join(format!("src.{}", std::process::id()));
    let written = std::fs::create_dir_all(&dir)
        .and_then(|()| std::fs::write(dir.join(format!("{FMT_MODULE}.ply")), FMT_SOURCE));
    if let Err(e) = written {
        return Err(unbuilt(format!(
            "its source could not be placed in `{}`: {e}",
            dir.display()
        )));
    }
...
fn unbuilt(why: String) -> Diagnostic {
    Diagnostic::error(
        ...
        format!("the `ply fmt` program could not be built: {why}"),
    )
    .primary(Span::DUMMY, "this is Ply's fault, not the program's")
    .note("the program is `crates/ply-cli/ply/fmt.ply`, built from the compiler this binary ships")
}
