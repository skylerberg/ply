...
//! lives in `crates/ply-cli-tests`, compiled at `opt-level = 0` in a package
//! of its own (see the profile comment in the root `Cargo.toml`). Without an
//! integration test here, that suite would find no `ply` to run.
