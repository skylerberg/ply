...
//!
//! A map is a value, not a node: there is no map literal in W2, so a definition
//! that builds one is a chain of calls to prelude builtins and normalizes like
//! any other. That is the claim here, and the pinned digest is what makes it a
//! claim about every run rather than about this one.
...
/// A builtin is a free reference by name, so nothing about `Map` gets a private
/// channel into the hash.
#[test]
...
/// Two definitions that build the same *value* by different insertion orders are
/// two computations, so they hash differently — and that is right. The map they
/// produce is one value; the programs that produce it are two, and the hash is
/// over the program.
#[test]
...
/// The declared `Map<k, v>` is part of the published signature, so swapping the
/// key and value types moves the hash even though the body is untouched.
#[test]
...
/// A pin, and therefore a claim across runs, processes and builds: a definition
/// whose body is a map is normalized to these bytes and to no others. A change
/// here is either a normalization change — which owes `FRONTEND_VERSION` a bump
/// — or a defect.
#[test]
