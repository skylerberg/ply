...
#
#   ./crates/ply-compiler-diff/tools/test-resolve.sh              # the in-language tests
#   ./crates/ply-compiler-diff/tools/test-resolve.sh --keep       # leave the project behind
set -euo pipefail
...
cp "$here"/lexer.ply "$here"/spine.ply "$here"/types.ply "$here"/patterns.ply \
   ...
   "$here"/derive.ply "$here"/tycore.ply "$here"/infer.ply "$here"/hash.ply "$work/"
