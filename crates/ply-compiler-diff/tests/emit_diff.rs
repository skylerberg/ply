...
use ply_compiler_diff::{byte_literal, reference_emit_dump};
...
impl Project {
    ...
    fn dumps_in(&self, names: &[String], inputs: &[Vec<u8>], ctors: &[String]) -> Vec<String> {
        let mut src = String::from(
            "// Generated by crates/ply-compiler-diff/tests/emit_diff.rs. Not checked in.\n\
             import emit (emit_dump_all)\n",
        );
        src.push_str(&format!(
            "fn builtins() -> List<Bytes> = [{}]\nfn ctors() -> List<Bytes> = [{}]\n",
            ply_compiler_diff::reference_builtins()
                .iter()
    ...
    fn dumps(&self, inputs: &[Vec<u8>]) -> Vec<String> {
        ...
        for (i, bytes) in inputs.iter().enumerate() {
            ...
            let ctors = ply_compiler_diff::reference_ctors(&[(
                "m".to_string(),
    ...
    fn bodies_in(&self, names: &[String], inputs: &[Vec<u8>], ctors: &[String]) -> String {
        let mut src = String::from(
            "// Generated by crates/ply-compiler-diff/tests/emit_diff.rs. Not checked in.\n\
             import emit (emit_bodies_reference)\n",
        );
        src.push_str(&format!(
            "fn builtins() -> List<Bytes> = [{}]\nfn ctors() -> List<Bytes> = [{}]\n",
            ply_compiler_diff::reference_builtins()
        ...
        src.push_str(
            "fn main() -> String = emit_bodies_reference(names(), srcs(), ctors(), builtins())\n",
        );
...
fn resolved_against_reference(
    ...
    let ctors = ply_compiler_diff::reference_ctors(&inputs);
