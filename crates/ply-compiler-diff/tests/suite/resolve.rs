...
use ply_compiler_diff::{golden, port, programs, records, reference_resolve_dump};
...
fn compare(label: &str, inputs: &[(String, Vec<(String, String)>)]) {
    ...
    for (name, program) in inputs {
        ...
        let reference = reference_resolve_dump(program);
        records_total += records(&reference).len();
        if let Err(report) = golden::check("resolve", name, &reference, &actual, first_difference) {
