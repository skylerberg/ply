...
fn dump_lit(out: &mut String, lit: &Lit) {
    match lit {
        ...
        // (`crates/ply-compiler/GAPS.md` §3), so no port could reproduce a value here; and the
...
/// of a differential can reproduce it. These are the names `crates/ply-compiler/ply/exprs.ply` already
