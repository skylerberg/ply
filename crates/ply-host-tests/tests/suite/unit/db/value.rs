...
/// Qualified, as a `Value` carries it.
...
/// Inference checks a perform's argument types, so an undeclared constructor cannot arrive.
#[test]
...
fn json_crosses_the_boundary_in_both_directions() {
    ...
    // A `Map` canonicalises key order, so the way back out is sorted.
    assert_eq!(back.render(), document.render());
}

/// A `Value::Ctor`'s identity is its program-wide name; no `match` can take apart a bare `Str`.
#[test]
fn a_json_document_carries_the_module_that_declares_it() {
    ...
    // The qualifier is what keeps two modules' `Array`s apart.
