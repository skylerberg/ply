...
/// Kept out of `SERVER` so the green run stays green: each is a partial builtin at an input it refuses.
...
/// A `Bytes` literal that normalized unstably would re-run every test on every invocation.
#[test]
...
/// The literal tag carries a length, and a length written against the wrong cursor moves every hash after it.
#[test]
