...
reason. A crate whose `[lib]` says `test = false` keeps its unit tests in its
`-tests` package too, as the `unit` module of the `suite` target
(`tests/suite/unit/`; a package with no suite has a `unit` target instead); the
other crates' unit tests stay in `src/`.
...
| `crates/ply-ty/src/ty.rs` `conflicts_with` | test scheduling, silently — tests still pass, they just stop running concurrently, or start racing |
...
14. **`AssertionKind::RecursionLimit` classifies nothing.** Found 2026-08-24
    ...
    *"The message keeps the phrase 'recursion limit' so that ADR 0004's
    ...
    `ply-test-tests/tests/suite/unit/runner.rs`, `ply-eval-tests/tests/suite/unit/evaluator.rs`) all match the rendered
