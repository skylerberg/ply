...
reason. `ply-host`'s and `ply-eval`'s unit tests are in their `-tests` packages
too, as the `unit` target, with `[lib] test = false` behind them; the other
crates' unit tests stay in `src/`.
...
> asserts — `crates/ply-cli-tests/tests/suite/cli.rs:198
...
14. **`AssertionKind::RecursionLimit` classifies nothing.** Found 2026-08-24
    ...
    *"The message keeps the phrase 'recursion limit' so that ADR 0004's
    ...
    `ply-test-tests/tests/unit/runner.rs`, `ply-eval-tests/tests/unit/evaluator.rs`) all match the rendered
